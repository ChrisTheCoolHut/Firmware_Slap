...
def trace_function(func, proc_queue, ld_path=""):
    ...
    proc_queue.put(
        do_trace(start_addr, args, file_name=file_name, ld_path=ld_path)[1])
...
def check_actions(path):
    for action in path.history.actions:
        if isinstance(action, angr.state_plugins.sim_action.SimActionData) \
            ...
            action_value = path.solver.eval(
                action.actual_value,
                cast_to=bytes,
                extra_constraints=[path.regs.pc == b"CCCC"])
...
def do_trace(start_addr, args, file_name=None, ld_path=""):
    ...
    #Hook for cmd inj
    system_list = [
        'system', 'execv', 'execve', 'popen', 'execl', 'execle', 'execlp',
        'do_system'
    ]
    ...
    state = p.factory.call_state(start_addr,
                                 *clarip_vars,
                                 add_options=my_extra)
        ...
        if action.type != 'constraint':
            if action.action == 'read' or action.action == 'write':
                ...
                if check_val in action_data.decode('utf-8', 'ignore'):
                    ...
                    print("DATA      : {}".format(
                        repr(action_data).replace(check_val,
                                                  FAIL + check_val + ENDC)))
...
def get_corruption_location(proj, path, check_val="CCCC"):
    ...
    for action in path.history.actions:
        if action.type != 'constraint':
            if action.action == 'read' or action.action == 'write':
                action_data = path.se.eval(action.data, cast_to=bytes)
                if check_val in action_data.decode(
                        'utf-8',
                        'ignore') and action.bbl_addr not in bbl_addrs:
                    action_dict = {}
                    action_dict['BBL_ADDR'] = hex(action.bbl_addr)
                    action_dict['BBL_DESC'] = get_desc_and_dis(
                        proj, action.bbl_addr)
...
def pretty_print(ref, type, value):
    ...
    print_format = "{:<16} : {:<15} | {}"
    if ref.__class__ is not str and 'base' in ref.keys(
    ) and 'offset' in ref.keys():
...
def get_ref_value(ref, type, value):
    if ref.__class__ is not str and 'base' in ref.keys(
        ...
        better_base = "{}+{}".format(ref['base'], ref['offset'])
        return {"base": better_base, "type": type, "value": value}
    else:
        return {"base": ref, "type": type, "value": value}
...
def process(proj, simgr):
    ...
    if "vulnerable" in simgr.stashes.keys():
    ...
    elif "exploitable" in simgr.stashes.keys() and len(
        ...
        return_dict['Injected_Location'] = get_ref_value(
            solved_addr, "char *", path.globals['cmd'])
...
def get_desc_and_dis(proj, address):
    ...
    ret_dict['DISASSEMBLY'] = [
        str(x) for x in proj.factory.block(address).capstone.insns
    ]
    return ret_dict
