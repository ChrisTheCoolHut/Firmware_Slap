...
def get_es():
    return Elasticsearch([{'host': es_host, 'port': es_port}],  timeout=30, max_retries=10, retry_on_timeout=True)
...
def build_index(es, i_name, del_if_exists=False):
    ...
    if es.indices.exists(i_name) and del_if_exists:
        print("Deleteing index by name {}".format(
            i_name))
    ...
    if not es.indices.exists(i_name):
        ...
        print("Creating index by name {}".format(
            i_name))
