...
def check_iter_status(async_items, done_list):
    ...
    ret = [x.get(propagate=False) for x in [y for y in async_items if y.ready()] if not x.failed()]
...
def display_scores_list(done_list, per_row=2, plot_it=True):
    ...
    print(colored("[+] Current cluster scores (Cluster Count, Cluster Score)", 'white', attrs=['bold']))
    ...
    for item in temp_list:
        temp_string = format_item.format(
                item['count'], str(item['score'])[:6])
