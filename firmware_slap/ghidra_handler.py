...
def run_dump_functions(dirpath, base_name, file_name, script_path,
    ...
    dump_cmd = run_headless_command.format(dirpath, project_name, file_name,
                                           script_path, set_options,
                                           dump_funcs, output_file_path)
...
def get_function_information(file_name):
    ...
    run_dump_functions(dirpath, base_name, file_name, scripts_dir,
                       full_output_path)
...
def get_arg_funcs(file_name, useHiFunc=True):
    if useHiFunc:
        return [
            ...
            if len(x) > 0 and x['HiParameterCount'] > 0
        ]
    else:
        return [
            x for x in get_function_information(file_name)
