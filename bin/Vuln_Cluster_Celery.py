...
def main():
    ...
    if args.Vuln_Pickle:
        ...
        file_vulnerabilities = vd.process_file_or_folder(args.Directory, args.LD_PATH)
    ...
    for function_list in async_and_iter(async_get_sparse_file_data, all_files):
        ...
        all_trim_funcs.extend(fhc.trim_funcs(function_list,
            function_list[0]['file_name']))
    ...
    for x in range(1,len(all_scores)-2):
        ...
        if largest_dif < abs(scores[x] - scores[x+1]):
                ...
                large_index = x+1
        '''
    print("Largest drop at {} with {}".format(all_scores[large_index]['count'], largest_dif))
    ...
    for file_vuln in bugs:
        ...
        similar_list = get_functions_on_cluster(all_functions,
                vuln_cluster, Largest_Score_Drop['labels'])

        reduced_list = []
        for func in similar_list:
            func_distance = get_func_dist(file_vuln, func, 
                    function_distances, all_functions)
            reduced_list.append({
...
def get_func_dist(base_function, compare_function, distance_matrix, all_functions):
    ...
    for x in range(len(all_functions)):
        ...
        if 'offset' in all_functions[x].keys():
            if file_vuln['offset'] == all_functions[x]['offset']:
                if os.path.basename(file_vuln['file_name']) == os.path.basename(func['file_name']):
                        return x
    return None
...
def print_function(func):
    ...
    if 'Similar_Funcs' in func.keys():
        print(colored("Similar Functions With Distances", 'magenta', attrs=['bold']))
